...
$id = $_GET["id"];
$id1 = $_GET["id1"];
$select = "SELECT * FROM coursedetails1 WHERE id=$id1";
$result = mysqli_query($conn, $select);
$row = mysqli_fetch_assoc($result);
$coursename = $row["coursename"];
$coursetype = $row["coursetype"];
$select1 = "SELECT * FROM exercises WHERE id=$id";
...
if(isset($_POST["Submit"])){
    ...
    if(empty($exename) || empty($exetime)){
        ...
        if(empty($filename)){
            $update = "UPDATE exercises SET exercises_name='$exename',exercises_time='$exetime' WHERE id='$id'"; 
        }else{
            $update = "UPDATE exercises SET exercises_name='$exename',exercises_time='$exetime',exercises_video='$folder' WHERE id='$id'"; 
        }
        ...
        <div style="margin-top: -100px;" class="dash-content">
            ...
            <form class="row g-3" action="<?php htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post" enctype="multipart/form-data">
                ...
                while($row1 = mysqli_fetch_assoc($result1)){
                    ...
                    <input type="text" name="exercisesname" value="<?php echo $row1["exercises_name"] ?>" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" autocomplete="off">
                </div>
                <div class="mb-3">
                    <label for="exampleInputPassword1" class="form-label">Exercise Time</label>
                    <input type="text" name="exercisestime"  value="<?php echo $row1["exercises_time"] ?>" class="form-control" id="exampleInputPassword1" autocomplete="off">
                </div>
                <div class="mb-3">
                    <label for="formFile" class="form-label">Exercise Graphic</label>
                    <input name="exercisesgraphic" class="form-control"  value="<?php echo $row1["exercises_video"] ?>" type="file" id="formFile" autocomplete="off">
                </div>
                    <?php 
                }
                ?>
